...
// function onlyMiddleNums(){
// 	for(let i = 0; i < middleNums.length; i++){
// 		if(middleNums[i] > 0 && middleNums[i] < middleNums.length - 1){
// 			return middleNums[i]
// 		}
// 	}
// }
...
let me = {
	...
	age: 34,
	greeter: function(){
		alert(`Hello! My name is ${firstName} and I live in ${state}.`);
	}	
}
...
const filtered = myNumbers.filter(i => i > 100)

//////////////////PROBLEM 16////////////////////

// Next up, reduce to get the total of the array. call the new array 'total'

const total = myNumbers.reduce((total, current) => {
...
myNumbersIndex.forEach(function(element, array, index){
	alert(element[i].push())
})
...
let forTheLoveOfGeorge = notGeorge.map(i => 'George')
...
let enemies = people.filter((element, index, array) => {return element.friend === false})
